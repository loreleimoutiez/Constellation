...
class HumanAsset(BaseAsset):
    ...
    termination_date: Optional[datetime] = Field(
        None, description="Date of termination if applicable"
    )
    ...
    on_call_enabled: bool = Field(
        False, description="Whether person is in on-call rotation"
    )
    working_hours_start: Optional[time] = Field(
        None, description="Normal working hours start"
    )
    working_hours_end: Optional[time] = Field(
        None, description="Normal working hours end"
    )
    timezone: Optional[str] = Field(None, description="Primary timezone")

    # System access and security
    security_clearance: Optional[str] = Field(
        None, description="Security clearance level"
    )
    ...
    def __str__(self) -> str:
        ...
        return (
            f"Human: {self.display_name or self.name} ({self.employee_id or self.id})"
        )
...
class Role(BaseAsset):
    ...
    level: Optional[str] = Field(
        None, description="Role level (e.g., 'Senior', 'Lead', 'Principal')"
    )

    # Organizational context
    department: Optional[str] = Field(
        None, description="Department this role belongs to"
    )
    team_id: Optional[str] = Field(None, description="Primary team for this role")
    reports_to_role_id: Optional[str] = Field(
        None, description="Role this position reports to"
    )

    # Role characteristics
    is_management_role: bool = Field(
        False, description="Whether this is a management role"
    )
    is_on_call_role: bool = Field(
        False, description="Whether this role includes on-call duties"
    )
    requires_security_clearance: bool = Field(
        False, description="Whether role requires security clearance"
    )
    ...
    required_skills: List[str] = Field(
        default_factory=list, description="Required skill names"
    )
    preferred_skills: List[str] = Field(
        default_factory=list, description="Preferred skill names"
    )
...
class Team(BaseAsset):
    ...
    department: Optional[str] = Field(
        None, description="Department this team belongs to"
    )
    ...
    is_virtual: bool = Field(
        False, description="Whether this is a virtual/cross-functional team"
    )
...
class HumanSkillRelation(TimestampMixin):
    ...
    evidence_type: Optional[str] = Field(
        None, description="Type of evidence (certification, project, etc.)"
    )
