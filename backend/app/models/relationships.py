...
class Relationship(TimestampMixin):
    ...
    weight: float = Field(
        1.0, ge=0.0, le=10.0, description="Numeric weight for algorithms"
    )
    ...
    active: bool = Field(
        True, description="Whether the relationship is currently active"
    )
    verified: bool = Field(
        False, description="Whether the relationship has been verified"
    )
    auto_discovered: bool = Field(False, description="Whether discovered automatically")

    # Context and conditions
    conditions: Optional[str] = Field(
        None, description="Conditions when this relationship applies"
    )
    ...
    discovered_by: Optional[str] = Field(
        None, description="Who or what discovered this relationship"
    )
    discovered_at: Optional[datetime] = Field(
        None, description="When this relationship was discovered"
    )
