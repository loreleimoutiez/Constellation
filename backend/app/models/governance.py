...
class Policy(BaseAsset):
    ...
    approved_at: Optional[datetime] = Field(
        None, description="When policy was approved"
    )
    effective_date: Optional[datetime] = Field(
        None, description="When policy becomes effective"
    )
    ...
    frameworks: List[str] = Field(
        default_factory=list, description="Compliance frameworks (GDPR, SOX, etc.)"
    )
    ...
    exceptions_allowed: bool = Field(
        False, description="Whether exceptions are allowed"
    )

    # Documentation
    document_url: Optional[HttpUrl] = Field(
        None, description="Link to full policy document"
    )
...
class Risk(BaseAsset):
    ...
    vulnerability: Optional[str] = Field(
        None, description="Vulnerability being exploited"
    )
    ...
    operational_impact: Optional[str] = Field(
        None, description="Operational impact description"
    )
...
class Process(BaseAsset):
    ...
    # Documentation
    procedure_url: Optional[HttpUrl] = Field(
        None, description="Link to procedure document"
    )
    flowchart_url: Optional[HttpUrl] = Field(
        None, description="Link to process flowchart"
    )
    ...
    last_review: Optional[datetime] = Field(
        None, description="Last process review date"
    )
    next_review: Optional[datetime] = Field(None, description="Next review date")
    ...
    def __repr__(self) -> str:
        """Debug representation."""
        return (
            f"Process(id='{self.id}', name='{self.name}', type='{self.process_type}')"
        )
...
class Control(BaseAsset):
    ...
    objective: Optional[str] = Field(
        None, max_length=1000, description="Control objective"
    )
    description_text: Optional[str] = Field(
        None, description="Detailed control description"
    )
    ...
    effectiveness_rating: Optional[str] = Field(
        None, description="Control effectiveness rating"
    )
    ...
    control_families: List[str] = Field(
        default_factory=list, description="Control families"
    )

    # Testing and validation
    test_frequency: Optional[str] = Field(
        None, description="How often control is tested"
    )
...
class Vendor(BaseAsset):
    ...
    relationship_start: Optional[datetime] = Field(
        None, description="Start of business relationship"
    )
    relationship_status: Optional[str] = Field(
        None, description="Current relationship status"
    )

    # Services and capabilities
    services_provided: List[str] = Field(
        default_factory=list, description="Services provided"
    )
    capabilities: List[str] = Field(
        default_factory=list, description="Vendor capabilities"
    )
    ...
    security_assessments: List[str] = Field(
        default_factory=list, description="Security assessment results"
    )

    # Financial information
    annual_spend: Optional[float] = Field(
        None, ge=0, description="Annual spend with vendor"
    )
    payment_terms: Optional[str] = Field(None, description="Payment terms")

    # Performance metrics
    sla_compliance: Optional[float] = Field(
        None, ge=0, le=100, description="SLA compliance percentage"
    )
    performance_rating: Optional[str] = Field(
        None, description="Overall performance rating"
    )
...
class Contract(BaseAsset):
    ...
    contract_number: Optional[str] = Field(
        None, description="Contract reference number"
    )
    ...
    # Timeline
    effective_date: Optional[datetime] = Field(
        None, description="Contract effective date"
    )
    ...
    contract_value: Optional[float] = Field(
        None, ge=0, description="Total contract value"
    )
    ...
    # Service levels
    sla_terms: Dict[str, Any] = Field(
        default_factory=dict, description="SLA terms and targets"
    )
    penalties: Dict[str, Any] = Field(
        default_factory=dict, description="Penalty clauses"
    )

    # Documentation
    contract_url: Optional[HttpUrl] = Field(
        None, description="Link to contract document"
    )
