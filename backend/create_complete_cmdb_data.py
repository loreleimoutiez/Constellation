...
NAMES_BY_TYPE = {
    "servers": [
        "srv-web-prod-01", "srv-web-prod-02", "srv-app-dev-01", "srv-app-dev-02", 
        "srv-db-prod-01", "srv-db-prod-02", "srv-backup-01", "srv-monitoring-01", 
        "srv-file-share-01", "srv-mail-prod-01", "srv-dns-01", "srv-proxy-prod-01", 
        "srv-jenkins-dev-01", "srv-gitlab-prod-01", "srv-elk-stack-01"
    ],
    "workstations": [
        "PC-DEV-001", "PC-DEV-002", "PC-MARKETING-001", "PC-RH-001", 
        "LAPTOP-SALES-001", "LAPTOP-ADMIN-001", "MAC-DESIGN-001", "PC-SUPPORT-001"
    ],
    "network_devices": [
        "SW-CORE-PROD-01", "SW-ACCESS-01", "RTR-BORDER-01", "FW-PERIMETER-01", 
        "LB-WEB-PROD-01", "AP-WIFI-FLOOR1-01"
    ],
    "storage_devices": [
        "SAN-PROD-01", "NAS-BACKUP-01", "TAPE-LIBRARY-01", "CLOUD-STORAGE-AWS"
    ],
    "applications": [
        "Constellation CMDB", "ERP SAP", "CRM Salesforce", "Active Directory", 
        "JIRA Service Desk", "Confluence", "GitLab CE", "Monitoring Zabbix",
        "Slack", "Microsoft Teams", "Office 365", "Antivirus Kaspersky"
    ],
    "system_software": [
        "Windows Server 2019", "Ubuntu 20.04", "Red Hat Enterprise Linux", 
        "VMware vSphere", "Docker Engine", "Kubernetes", "Apache Web Server", "Nginx"
    ],
    "middleware": [
        "Apache Tomcat", "JBoss EAP", "WebLogic Server", "RabbitMQ", "Apache Kafka"
    ],
    "databases": [
        "PostgreSQL Production", "MySQL Development", "MongoDB Analytics", 
        "Redis Cache", "Neo4j Graph DB", "Oracle ERP DB", "SQL Server CRM", "InfluxDB Metrics"
    ],
    "services": [
        "Service Web HTTP", "Service API REST", "Service Mail SMTP", 
        "Service DNS", "Service DHCP", "Service VPN"
    ],
    "employees": [
        "Marie Dubois (DSI)", "Pierre Martin (DevOps)", "Sophie Laurent (Dev Senior)", 
        "Jean Durand (Admin Sys)", "Camille Bernard (Chef Projet)", "Lucas Moreau (Analyste)",
        "Emma Petit (Support)", "Hugo Simon (Architecte)", "Léa Roux (QA)", "Thomas Blanc (Sécurité)"
    ],
    "teams": [
        "Équipe DevOps", "Équipe Sécurité", "Équipe Infrastructure", 
        "Équipe Support N2", "Équipe Développement"
    ],
    "contractors": [
        "Consultant ACME Corp", "Prestataire SecurIT", "Freelance Dev Mobile"
    ],
    "policies": [
        "Politique de Sécurité IT", "Politique RGPD", "Politique Backup", "Charte Utilisateur"
    ],
    "procedures": [
        "Procédure Incident Management", "Guide Déploiement", 
        "Manuel Utilisateur CMDB", "Documentation Architecture"
    ],
    "certifications": [
        "Certification ISO 27001", "Certification ITIL v4", "Certification AWS"
    ],
    "software_licenses": [
        "Licence Microsoft Office 365", "Licence VMware vSphere", 
        "Licence Adobe Creative Suite", "Licence Atlassian Suite", 
        "Licence Windows Server", "Licence Oracle Database"
    ],
    "contracts": [
        "Contrat Maintenance Dell", "Contrat Support Microsoft", "Contrat Cloud AWS"
    ]
}
...
RELATIONSHIP_TYPES = [
    ...
    "DEPENDS_ON", "RUNS_ON", "HOSTS", "CONNECTS_TO", "INSTALLED_ON", "USES",
    "PRODUCES", "CONSUMES", "PROCESSES", "STORES",
    
    # Relations humaines
    "OWNS", "RESPONSIBLE_FOR", "ACCOUNTABLE_FOR", "CONSULTED_FOR", 
    "INFORMED_FOR", "MEMBER_OF", "HAS_ROLE", "HAS_SKILL",
    
    # Relations de gouvernance
    "GOVERNED_BY", "SUBJECT_TO", "COMPLIES_WITH", "PROTECTS", "COVERS",
    
    # Relations business
    "OUTSOURCED_TO", "PROVIDED_BY", "COVERED_BY", "CONTRACTED_WITH",
    
    # Relations de connaissance
    "DOCUMENTS", "KNOWS", "REFERENCES"
]

def generate_human_attributes():
    """Génère des attributs spécifiques aux humains"""
    departments = ["IT", "RH", "Finance", "Marketing", "Operations", "Direction", "Legal", "Support"]
    job_titles = [
        "Ingénieur DevOps", "Administrateur Système", "Développeur Full Stack", 
        "Chef de Projet", "Analyste Business", "Directeur IT", "Architecte Solution",
        "Consultant Sécurité", "Support N2", "Data Analyst"
    ]
    skills = [
        "Python", "Docker", "Kubernetes", "AWS", "Linux", "Monitoring", 
        "Security", "Networking", "Database", "CI/CD", "Terraform", "Ansible"
    ]
    ...
    return {
        ...
        "certifications": random.choice([
            [], ["CISSP"], ["AWS Certified"], ["ITIL v4"], 
            ["CISSP", "ITIL v4"], ["AWS Certified", "Docker Certified"]
        ]),
...
def generate_license_attributes():
    ...
    vendors = ["Microsoft", "VMware", "Oracle", "Adobe", "Atlassian", "AWS", "Google", "Cisco"]
    license_types = ["Per User", "Per Device", "Site License", "Enterprise", "Subscription", "Perpetual"]
    support_levels = ["Basic", "Standard", "Premium", "Enterprise", "24/7"]
    
    seats_total = random.choice([5, 10, 25, 50, 100, 250, 500, 1000])
    seats_used = random.randint(1, min(seats_total, seats_total - random.randint(0, seats_total // 4)))
    ...
    return {
        ...
        "renewal_date": (datetime.now() + timedelta(days=random.randint(30, 730))).strftime("%Y-%m-%d"),
        "support_level": random.choice(support_levels)
    }

def generate_policy_attributes():
    """Génère des attributs spécifiques aux politiques"""
    policy_types = ["security", "compliance", "operational", "business", "legal", "technical"]
    approval_statuses = ["draft", "pending", "approved", "retired", "under_review"]
    frameworks = ["ISO27001", "RGPD", "ITIL", "COBIT", "NIST", "PCI-DSS", "SOX"]
    departments = ["IT", "Legal", "RH", "Finance", "Operations", "Security", "Compliance"]
    
    return {
        "policy_type": random.choice(policy_types),
        "approval_status": random.choice(approval_statuses),
        "approval_date": fake.date_between(start_date='-2y', end_date='today').strftime("%Y-%m-%d") if random.choice([True, False]) else "",
        "review_date": (datetime.now() + timedelta(days=random.randint(90, 730))).strftime("%Y-%m-%d"),
...
def create_asset(asset_type, template, name):
    ...
    if template["category"] == "tangible":
        asset_data.update({
            ...
            "lifecycle_state": random.choice(["ACTIVE", "PLANNED", "DEPRECATED", "RETIRED"]),
        ...
        if asset_type in ["servers", "applications", "workstations", "network_devices"]:
            asset_data.update({
                ...
                "vendor": random.choice(["Dell", "HP", "Cisco", "VMware", "RedHat", "Microsoft", "IBM", "Oracle"]),
                "model": f"Model-{random.randint(1000, 9999)}",
                "location": random.choice([
                    "Datacenter Paris", "Datacenter Lyon", "Cloud AWS EU-West-1", 
                    "Office Headquarters", "Site Remote", "Azure West Europe"
                ])
            })
    ...
    # Attributs pour assets intangibles
        ...
        elif intangible_type in ["governance", "documentation", "certification", "contract"]:
